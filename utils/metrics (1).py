...
class MetricsCalculator:
    ...
    def compare_models(self, model_names):
        ...
        for model_name in model_names:
            ...
            resource_utilization[model_name] = {
                ...
                'cpu_usage': np.random.uniform(30, 85),  # percentage
                'inference_time': np.random.uniform(10, 100)  # milliseconds
            }
            
            # Model complexity
            complexity[model_name] = {
                'parameters': np.random.randint(10000, 1000000),
                'depth': np.random.randint(3, 20),
                'interpretability': np.random.uniform(0.3, 0.9)
            }
    ...
    def get_data_quality_metrics(self):
        ...
        return {
            ...
            'consistency': np.random.uniform(0.85, 0.94),
            'timeliness': np.random.uniform(0.90, 0.98),
            'validity': np.random.uniform(0.87, 0.95),
            'duplicate_rate': np.random.uniform(0.01, 0.05),
            'missing_values': np.random.uniform(0.02, 0.08),
            'outliers': np.random.uniform(0.01, 0.03)
        }
